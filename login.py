...
def signin():
    ...
    if username in r.keys() and password==r[username]:
        ...
        Label(
            screen, text="Hello, User!", bg="#fff", font=("Calibri(Body)", 50, "bold")
        ).pack(expand=True)
        screen.mainloop()
...
def signup_command():
    ...
    def on_exit(e):
        if confirm_code.get() == "":
            confirm_code.insert(0,"Re-Enter Password")
        
    confirm_code = Entry(frame, width=25,fg='black', border= 0, bg='white',font= ("Microsoft Yahei UI Light", 11))
    confirm_code.place(x=30,y=220)
    confirm_code.insert(0, "Re-Enter Password")
    confirm_code.bind("<FocusIn>", on_enter)
    confirm_code.bind("<FocusOut>", on_exit)
...
sign_up = Button(
    frame, width=6, text="Sign up", border=0, bg="white", cursor="hand2", fg="#57a1f8", command=signup_command)
